...
export async function createSupplierProduct(businessId: string, data: Record<string, unknown>) {
  const [sp] = await db.insert(supplierProduct).values(data as any).returning()
  return sp
}

export async function updateSupplierProduct(businessId: string, id: string, data: Record<string, unknown>) {
  const [updated] = await db.update(supplierProduct)
    .set(data as any)
    .where(eq(supplierProduct.id, parseInt(id)))
    .returning()
