...
import { createSupplierSchema, updateSupplierSchema, createSupplierProductSchema, updateSupplierProductSchema, createPurchaseOrderSchema, updatePurchaseOrderSchema, receiveGoodsSchema, supplierPaymentSchema } from './purchases.schemas'
...
purchases.get('/suppliers/:id', vValidator('param', idParamSchema), async (c) => {
  const businessId = c.get('businessId')
...
purchases.put('/suppliers/:id', vValidator('param', idParamSchema), vValidator('json', updateSupplierSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
...
purchases.delete('/suppliers/:id', vValidator('param', idParamSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
...
purchases.get('/supplier-products', vValidator('query', v.object({ supplierId: v.optional(idSchema) })), async (c) => {
  const businessId = c.get('businessId')
...
purchases.put('/supplier-products/:id', vValidator('param', idParamSchema), vValidator('json', updateSupplierProductSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
...
purchases.delete('/supplier-products/:id', vValidator('param', idParamSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
...
purchases.get('/orders/:id', vValidator('param', idParamSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
...
purchases.put('/orders/:id', vValidator('param', idParamSchema), vValidator('json', updatePurchaseOrderSchema), async (c) => {
  const businessId = c.get('businessId')
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
...
purchases.post('/orders/:id/receive', vValidator('param', idParamSchema), vValidator('json', receiveGoodsSchema), async (c) => {
  const businessId = c.get('businessId')
  const userId = c.get('userId')
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
...
purchases.post('/payables/:id/payments', vValidator('param', idParamSchema), vValidator('json', supplierPaymentSchema), async (c) => {
  const businessId = c.get('businessId')
  const userId = c.get('userId')
  const { id } = c.req.valid('param')
  const body = c.req.valid('json')
